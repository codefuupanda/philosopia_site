...
- **Build tool: Vite** (migrated from Create React App 2026-07-10). `vite.config.js` holds the React plugin, the `@` → `src/` alias (mirrors `jsconfig.json`, kept for editor intellisense), and dev-server port 3000. `index.html` lives at the package root (Vite convention); build output is `dist/`. Env vars must be prefixed `VITE_` and read via `import.meta.env` — `process.env.*` is not available in the browser bundle.
...
- **Relation graph viz** (`src/components/RelationshipGraph.jsx` + `src/pages/GraphPage.jsx` at `/{lang}/graph`, added 2026-07-10): `react-force-graph-2d` over `useGraphNetwork()` (the `/philosophers/graph/network` endpoint). Nodes are era-colored (hue map over the canonical periodIds — see Gotchas), sized by degree; labels appear on zoom or hover. Canvas colors are resolved from the shadcn CSS variables at render, keyed on `theme` from `ThemeContext`, so the graph re-colors on toggle. Hover highlights a node's neighborhood and dims the rest; inferred links draw **dashed**; arrows point influencer → influenced; click navigates to the profile. The graph mutates its data, so the hook's response is deep-copied before being handed to the lib. Known dev-console noise: `react-kapsule` (the lib's wrapper) triggers React's "deps array changed size" warning — library-internal, dev-only, no production impact.
...
- In Docker the frontend is built with `VITE_API_BASE_URL=/api` and nginx proxies `/api/` to the API container (frontend waits for the API healthcheck). The Dockerfile copies Vite's `dist/` output (not CRA's `build/`).
- Local dev: API on 5000, frontend dev server on 3000; nginx serves on 80 in production.
...
⚠️ The IAM access key currently in `.env` was shared in a chat session on 2026-07-02 for expedited setup — **rotate it** (create a new key in IAM, update `.env`, delete the old one).

**`philosopia-site/.env`**:
```
VITE_API_BASE_URL=http://localhost:5000/api
```
(The CRA-era `REACT_APP_API_BASE_URL` and the `NODE_OPTIONS=--openssl-legacy-provider` workaround are gone — Vite needs neither.)
...
- **macOS port 5000:** AirPlay Receiver (`ControlCe` process) squats port 5000 on this machine. If the API won't bind, disable AirPlay Receiver or check `lsof -nP -iTCP:5000`. For throwaway verification runs, `PORT=5050 node server.js` + temporarily pointing `philosopia-site/.env` at 5050 works (restore it afterwards).
...
- **Rotate the default admin password** — `scripts/createAdmin.js` seeds `admin/password123`; change it before any public launch (the auth guard and JWT hardening are done, the credential is the remaining hole).
...
| Philosophers — timeline view (era sidebar, scroll-spy, detail modal) | ✅ Done |
...
| Relation graph visualization (`/{lang}/graph`, force-directed, hover-highlight, era-colored) | ✅ Done (2026-07-10) |
